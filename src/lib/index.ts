...
export function encryptText(message: Buffer, key: Buffer, spec: Spec): PasteData {
  return encrypt(message, key, spec);
}

export function decryptText(message: string, key: Buffer, adata: Adata): Buffer {
...
export class PrivatebinClient extends Api {
  ...
  public async sendText(text: string, key: Buffer, options: Options): Promise<Response> {
    ...
    const pasteData = textToBuffer(text, compression);
    const chiperPaste = encryptText(Buffer.from(pasteData), key, spec);
  ...
  public async getText(id: string, key: Buffer): Promise<Paste> {
    ...
    const paste = decryptText(response.ct, key, response.adata);
