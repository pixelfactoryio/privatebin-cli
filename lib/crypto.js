...
const encrypt = (message, masterkey, spec) => {
    const iv = crypto.randomBytes(16);
    const salt = crypto.randomBytes(8);
    const key = deriveKey(masterkey, salt, spec.iter);
    const adata = [
        [iv.toString('base64'), salt.toString('base64'), spec.iter, spec.ks, spec.ts, spec.algo, spec.mode, spec.compression], 'plaintext', 0, 0
    ];
    // AES 256 GCM Mode
...
const decrypt = (data, masterkey, adata) => {
